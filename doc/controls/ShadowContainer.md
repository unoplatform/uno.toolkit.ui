...
`ShadowContainer` is a control that provides the ability to layer both inner and outer shadows to its content. The `ShadowContainer` can host any number of `Shadow` objects, which support properties such as Color, Offset, Opacity, BlurRadius, Spread, etc.
...
In the following example, taken from the Shadow Builder sample within the [Uno Gallery](https://gallery.platform.uno/), we are presented with a small control that allows for the dynamic creation and modification of its root `ShadowContainer`’s `Shadows` collection. Each section in the list corresponds to a `Shadow` that is currently being drawn on its parent container. The user can add, remove, and modify the properties of each shadow, and the changes will be reflected in real-time.

![shadow builder example](../assets/shadow-builder.gif)

Taking a closer look at this Shadow Builder sample, we can see that the individual components within the view also make use of the `ShadowContainer` to produce a Neumorphic visual style to the control. [Neumorphism](https://neumorphism.io/) is a design style where elements are usually the same color as the background and are only distinguished by shadows and highlights surrounding the element. This gives the elements the appearance that they are protruding from the background, or that they are dented into it. Notice that each `TextBox` in the Shadow Builder appears sunken into its background, as well as the elevated `Button`s appearing indented upon being pressed.  


With the power of `ShadowContainer` to create rich shadows and highlights, we can produce interesting new styles for controls such as the Neumorphic style that we see in example above. To illustrate these capabilities, the Uno Gallery now hosts a new set of Neumorphic styles for most controls, including those from Uno Toolkit. These styles can be found in the [Uno Gallery GitHub repo](https://github.com/unoplatform/Uno.Gallery/blob/master/Uno.Gallery/Uno.Gallery.Shared/Views/Styles/NeumorphicStyles.xaml), a small subset of which can be seen below:

![neumorphic styles](../assets/neumorphic-styles.png) 
