...
namespace Uno.Toolkit.UI.Material
{
	...
	public class MaterialToolkitTheme : ResourceDictionary
	{
		...
		{
			if (d is MaterialToolkitTheme { _isFontOverrideMuted: false } toolkitTheme)
			{
		...
		{
			if (d is MaterialToolkitTheme { _isColorOverrideMuted: false } toolkitTheme)
			{
				toolkitTheme.UpdateSource();
			}
		}
		...
		private void UpdateSource()
		{
			...
			Source = new Uri($"ms-appx:///{ToolkitMaterialPackageName}/Generated/mergedpages.{PackageNameSuffix}.v2.xaml");

#if !HAS_UNO
			MergedDictionaries.Add(new MaterialTheme(null, null));
#else
