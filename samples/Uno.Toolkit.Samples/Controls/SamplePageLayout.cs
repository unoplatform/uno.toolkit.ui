...
namespace Uno.Toolkit.Samples
{
	public partial class SamplePageLayout : ContentControl
	{
		...
		private IReadOnlyCollection<LayoutModeMapping> LayoutModeMappings => new List<LayoutModeMapping>
		{
			...
			new LayoutModeMapping(Design.Simple, () => !IsDesignAgnostic, _simpleRadioButton, _stickySimpleRadioButton, VisualStateSimple, SimpleTemplate),
		...
		private void ApplySimpleThemeOverrides()
		{
			...
			// Prefer SimpleTemplate when defined, otherwise fall back to existing templates.
			if (!IsDesignAgnostic && DesignAgnosticTemplate == null)
			{
				DesignAgnosticTemplate = SimpleTemplate ?? M3MaterialTemplate ?? MaterialTemplate ?? CupertinoTemplate;
			}
