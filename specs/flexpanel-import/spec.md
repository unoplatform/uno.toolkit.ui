...
- [x] **P3 — Adapter.** `FlexPanel.cs` + `FlexPanel.Properties.cs` per D3/D4/D5. Tier-2 tests alongside, red/fix/green. ✅ **24 tests added (21 tier-2 + 3 tier-3), 898 passed / 0 failed / 57 skipped unfiltered** in Release on desktop/Skia, 0 warnings. Tier 3 was pulled forward from P4 and earned it — it caught a real leak: `SyncYogaTree` evicted removed children from the node caches but left its *scratch* collections populated, so a panel that never laid out again pinned them; the clears now sit in `finally` blocks (D9/NFR-4). Three deviations from upstream are recorded in `progress.md`, including the `Get/SetFlexMinWidth` accessor fix — upstream registers the DP as `"FlexMinWidth"` but names the accessors `Get/SetMinWidth`, which XAML cannot bind against. FR-9 re-verified: the `AutoLayout` diff against the fork point is empty.
- [ ] **P4 — Cross-platform validation.** Tier 1 + 2 on the Skia heads: desktop ✅, **WASM and Skia mobile outstanding**. Rounding matrix (risk 1) not run. The tier-3 leak test already landed in P3. Every result recorded under P3/P5 is desktop/Skia only, so this is the phase that turns them into the FR-8 claim.
